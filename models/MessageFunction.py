...
class MessageFunction(nn.Module):
    ...
    def __set_message(self, message_def, args={}):
        ...
        if self.m_function is None:
            print(
                "WARNING!: Message Function has not been set correctly\n\tIncorrect definition "
                + message_def
            )
            quit()
        ...
        init_parameters = {
            ...
            self.m_definition, lambda x: (nn.ParameterList([]), nn.ModuleList([]), {})
        )
    ...
    def m_ggnn(self, h_v, h_w, e_vw, opt={}):
        ...
        m = Variable(
            torch.zeros(h_w.size(0), h_w.size(1), self.args["out"]).type_as(h_w.data)
        )
        ...
        for w in range(h_w.size(1)):
            if torch.nonzero(e_vw[:, w, :].data).size():
                for i, el in enumerate(self.args["e_label"]):
                    ...
                    parameter_mat = self.learn_args[0][i][None, ...].expand(
                        h_w.size(0),
                        self.learn_args[0][i].size(0),
                        self.learn_args[0][i].size(1),
                    )

                    m_w = torch.transpose(
                        torch.bmm(
                            torch.transpose(parameter_mat, 1, 2),
                            torch.transpose(torch.unsqueeze(h_w[:, w, :], 1), 1, 2),
                        ),
                        1,
                        2,
                    )
    ...
    def init_ggnn(self, params):
        ...
        learn_args.append(
            nn.Parameter(
                torch.randn(len(params["e_label"]), params["in"], params["out"])
            )
        )
    ...
    def init_mpnn(self, params):
        ...
        # Define a parameter matrix A for each edge label.
        nnet = NNet(n_in=params["edge_feat"], n_out=(params["in"] * params["out"]))
        learn_modules.append(nnet)
